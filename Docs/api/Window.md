...
**Inheritance:** SpawnJSObject -> EventTarget -> Window  
**MDN Reference:** [Window on MDN](https://developer.mozilla.org/en-US/docs/Web/API/Window)  
**Source:** `SpawnDev.SpawnJS/JSObjects/DOM/Window.cs`

> The Window interface represents a browser window containing a DOM document. It provides access to the document, navigator, location, history, screen, storage, crypto, and performance APIs. In SpawnDev.SpawnJS, the Window instance for the current scope is available via `SpawnJSRuntime.WindowThis` when running in a window context.
