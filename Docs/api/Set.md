...
| `ForEach(ActionCallback<TValue, TValue, Set<TValue>> callbackFn, SpawnJSObject thisArg)` | `void` | Calls callbackFn once for each value present in the Set object, in insertion order. If a thisArg parameter is provided, it will be used as the this value for each invocation of callbackFn. |
