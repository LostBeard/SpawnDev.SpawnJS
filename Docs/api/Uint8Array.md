...
**Inheritance:** `SpawnJSObject` -> `TypedArray` -> `TypedArray<byte>` -> `Uint8Array`  
**MDN Reference:** [Uint8Array - MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array)

> The `Uint8Array` typed array represents an array of 8-bit unsigned integers. This is the most commonly used typed array for binary data in web applications and serves as the bridge between .NET `byte[]` and JavaScript binary data. SpawnDev.SpawnJS provides explicit cast operators for seamless conversion between `byte[]` and `Uint8Array`, plus `Create` factory methods for efficient copies via `HeapView`.
...
| `Uint8Array(SpawnJSObjectReference _ref)` | Deserialization constructor. |
