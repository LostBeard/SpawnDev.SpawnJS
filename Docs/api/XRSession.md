...
**Namespace:** `SpawnDev.SpawnJS.JSObjects.WebXR`  
**Inheritance:** `SpawnJSObject` -> `EventTarget` -> `XRSession`  
...
|---|---|
| `XRSession(SpawnJSObjectReference _ref)` | Deserialization constructor. Obtained via `XRSystem.RequestSession()`. |
