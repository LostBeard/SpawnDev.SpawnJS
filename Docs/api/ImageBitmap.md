...
**Inheritance:** `SpawnJSObject` -> `ImageBitmap`  
...
|-----------|-------------|
| `ImageBitmap(SpawnJSObjectReference _ref)` | Deserialization constructor - wraps an existing JS object reference. |
...
```csharp
// Create from various image sources via SpawnJSRuntime
