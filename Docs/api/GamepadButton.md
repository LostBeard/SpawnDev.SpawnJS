...
> The GamepadButton interface defines an individual button of a gamepad or other controller, allowing access to the current state of different types of buttons available on the control device. GamepadButton, when serialized via JSON.stringify(), returns and empty object {} so a SpawnJSObject is used even though the values are static https://developer.mozilla.org/en-US/docs/Web/API/GamepadButton

## Constructors

| Signature | Description |
|---|---|
| `GamepadButton(SpawnJSObjectReference _ref)` | Deserialization constructor |
