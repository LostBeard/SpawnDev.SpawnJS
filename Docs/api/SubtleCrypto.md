...
**Inheritance:** `SpawnJSObject` -> `SubtleCrypto`  
...
|---|---|
| `SubtleCrypto(SpawnJSObjectReference _ref)` | Deserialization constructor. Not directly instantiated - obtain via `Crypto.Subtle`. |
...
| Method | Return Type | Description |
|---|---|---|
| `ExportKey<T>(string format, CryptoKey key)` | `Task<T>` | Exports a key. `T` is `ArrayBuffer` for `"raw"`/`"pkcs8"`/`"spki"`, or `SpawnJSObject` for `"jwk"`. |
...
| `ExportKeyJwk(CryptoKey key)` | `Task<SpawnJSObject>` | Exports in JSON Web Key format. |
