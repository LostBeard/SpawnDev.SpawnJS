...
| `AsNativeView()` | `SpawnJSObject` | Returns the appropriate JS view - TypedArray for arrays, decoded string for strings. |
...
| `ToNativeView()` | `SpawnJSObject` | Creates a copy as the appropriate JS type. |
...
// The pattern used throughout SpawnDev.SpawnJS for fast data transfer:
