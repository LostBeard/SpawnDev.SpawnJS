...
ActionEvent properties are typically defined on SpawnJSObject subclasses like this:
...
- **Every `+=` MUST have a matching `-=` before the SpawnJSObject is disposed.** Failing to unsubscribe before disposal will cause the JavaScript callback to persist, leading to memory leaks and potential calls into disposed .NET objects that trigger Blazor error UI.
