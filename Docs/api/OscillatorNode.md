...
**Inheritance:** `SpawnJSObject` -> `EventTarget` -> `AudioNode` -> `AudioScheduledSourceNode` -> `OscillatorNode`  
...
| `OscillatorNode(SpawnJSObjectReference _ref)` | Deserialization constructor. |
